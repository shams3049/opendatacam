...
if [[ -z $VIDEO_INPUT ]]; then
  ...
  echo 'Using video input value from environment variable: '$VIDEO_INPUT
  jq '.VIDEO_INPUT = env.VIDEO_INPUT' config.json > "tmp" && mv "tmp" config.json
fi

if [[ -z $DETECTION_ENGINE ]]; then
  a=$(cat config.json | jq '.DETECTION_ENGINE')
  echo 'Using detection engine value from config.json: '$a
else
  echo 'Using detection engine value from environment variable: '$DETECTION_ENGINE
  jq '.DETECTION_ENGINE = env.DETECTION_ENGINE' config.json > "tmp" && mv "tmp" config.json
fi

if [[ -z $DEEPSTREAM_HOST ]]; then
  a=$(cat config.json | jq '.DEEPSTREAM_CONFIG.host')
  echo 'Using DeepStream host value from config.json: '$a
else
  echo 'Using DeepStream host value from environment variable: '$DEEPSTREAM_HOST
  jq '.DEEPSTREAM_CONFIG.host = env.DEEPSTREAM_HOST' config.json > "tmp" && mv "tmp" config.json
fi
...
if [[ -z $INPUT_FILE ]]; then
  ...
  echo 'Using video input file value from environment variable: '$INPUT_FILE
...
if [[ -z $INPUT_USBCAM ]]; then
  ...
  echo 'Using video input usbcam value from environment variable: '$INPUT_USBCAM
...
if [[ -z $INPUT_REMOTE_CAM ]]; then
  ...
  echo 'Using video input remote_cam value from environment variable: '$INPUT_REMOTE_CAM
